...
# plt.figure()
plt.figure(figsize=(3.14,1.8), dpi=600)
plt.stem(x_a, y_a, label='CKV index n',linefmt='black', markerfmt='.', basefmt="black")  # Linha sólida para curva A
plt.stem(x_b, y_b, label='$f_{ref}$ index k',linefmt='red', markerfmt='.', basefmt="red")
...
plt.annotate('$T_{Out}$', xy=(0.42, 0.85), xytext=(0.71, 0.83),
             arrowprops=dict(facecolor='black', arrowstyle='<|-|>', lw=0.5), fontsize=6)

plt.annotate('$T_{ref}$', xy=(1, 1.35), xytext=(2.020, 1.33),
             arrowprops=dict(facecolor='black', arrowstyle='<|-|>', lw=0.5), fontsize=6)
...
# Adicionar legenda
plt.legend(loc='upper left',fontsize=5.8, facecolor='white', framealpha=1)
plt.xlabel('CKV')#,fontsize=12)
# plt.ylabel(fontsize=12)
# plt.ylabel(label1 + ' [dBc/Hz]')
# Mostrar o gráfico
# plt.grid(True)
plt.savefig(r'C:\Users\ander\OneDrive\Área de Trabalho\artigo\index_n_k.eps', bbox_inches='tight',format='eps')
plt.show()
