...
def TDC(tR , t_ckv ,TCKV_accumulator):
    global TDC_res , T0 , TDC_chains , RV_n , RV_k , NUM_ZEROS, AVG_FCKV
    ...
    TDC_BITS_AVG_TCKV = 15    #Bits de resolução para média do periodo CKV
...
FCW_I_bits = 8
FCW_F_bits = 24
# normalização da parte fracionária de FCW em N bits.
    ...
    for k in range(1 , TIME):
        ...
        while t_CKV[n] < t_R:
            ...
            if trk_bank_calib:
                ...
                t_CKV.append(t_CKV[n - 1] + T0)
        if trk_bank_calib:
            error_fractional[k] = TDC(t_R*2 , t_CKV[n - 1]*2 ,1 / np.sum(freq_array))
