...
def format_plot(name1, name2, sys1, sys2,  omega, margins=False, offset=0):
    ...
    desc = np.ones_like(omega1)
    desc[omega1<KNEE] = np.abs((omega1[omega1<KNEE]/KNEE)**(-ALPHA))
    desc[0] = 1
    j = 0
    #mag_dB1 += 10*np.log10(desc)
    for i in range (len(omega1)):
        if(omega1[i] < 68e3):
            mag_dB1[i] -= 30 * np.log10(1 + omega1[i])
            last_mag =  mag_dB1[i]
            diff = mag_dB1[i+1] - (mag_dB1[i]) 
            j = i
        elif(omega1[i] > 68e3 and omega1[i] < 87e3):
             mag_dB1[i] += -diff - 10 * np.log10(1 + omega1[i]) + offset +3.3
             last_mag =  mag_dB1[i]
             diff2 = mag_dB1[i+1] - (mag_dB1[i]) 
        else:
            mag_dB1[i] += -diff2 # -diff2 + 20 * np.log10(1 + omega1[i]/omega1[j]) - 14.567 #-- (mag_dB1[j+1] - mag_dB1[j])
    
    #mag_dB1[j-50] -= 1
    #mag_dB1[j+50] -= 2
    # print("diff", diff, "last_mag", last_mag, "mag +1", mag_dB1[j+1], "diff2", diff2)
    print("diff", diff, "last_mag", last_mag)
    ...
    # plt.legend()
    # legenda = plt.legend(facecolor='white', framealpha=1)
    plt.grid()
    # legenda = plt.gca().get_legend()
    return plt
...
if __name__ == "__main__":
   ...
   # format_plot("$H_{TDC}$", "$H_{TDC}$ + IRR", Hcl_TDC, Hcl_TDC_irr, w, margins=True)
    ...
    #plt.figure()
    num = [1]
    den = [1, 0,0]  # Denominador é s
    tf = control.TransferFunction(num, den)
    Hcl_ADPLL = Hcl_DCO * Hcl_TDC
    Hcl_ADPLL_irr = Hcl_DCO_irr + Hcl_TDC_irr + Hcl_ref_irr
    pl = format_plot("Ruído de fase do DCO", "Ruído de fase do ADPLL + IRR", Hcl_ADPLL, Hcl_ADPLL_irr, w, margins=False, offset= 45)
    
    plt.savefig(r'C:\Users\ander\OneDrive\Área de Trabalho\Imagens\s_domain_phase_noise_loop_filter.eps', bbox_inches='tight', format='eps')
    # plt.figure()
    ...
    # Xdb_o , f = fun_calc_psd(mag , fr , 10 , 1)
    # plt.figure()
