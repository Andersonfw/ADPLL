...
'''
        DEFINIÇÕES GERAIS
'''
F0 = 2000e6  # frequência central de ajuste do DCO
...
deltaf = 100e6#3.5e6  # offset frequency
...
jitter = [0]  # jitter noise ( non Accumulative)
wander = [0]  # wander noise upconverted Thermal Noise, Accumulative)
jitter_ran = np.random.randn ((int(FCW) + 100)*TIME)
Wander_ran = np.random.randn( (int(FCW) + 100)*TIME)
...
if __name__ == "__main__":
    ...
    for k in range(1, TIME):
        ...
        while t_CKV[n] < t_R:
            ...
            jitter.append(jitter_ran[n] * Jt_noise)
            wander.append(Wander_ran[n] * Wt_noise)
    ...
    x = np.array(t_CKV[600*70:])
    # x= (np.array(freqs[130:]))

    # x = x*2*np.pi*(1/FREF)
    Xdb_o, f = fun_calc_psd(x, FREF, 8e3, 1e3)
    # Xdb_o, f = fun_calc_psd((x_t), N*f, 100e3, 10e3)
    ...
    vin = t_CKV[100*70:]
    # vin = 1/vin
    phase = signal.lfilter([Ts * 2 * np.pi * kv, -Ts * 2 * np.pi * kv], [1, -1], vin - np.mean(vin))

    window_length = int(np.floor(len(phase) / num_segments))

    f, PSDphase = signal.welch(phase, fs=1 /Ts, nperseg=window_length, noverlap=window_length/2, nfft=window_length,
                               return_onesided=True)
    plt.figure()
    plt.semilogx(f, 10*np.log10(PSDphase))
    plt.xlabel('Frequency [Hz]')
    plt.ylabel('Phase Noise [dBc/Hz]')
    plt.title('Phase Noise Analysis')
    plt.grid(True)
    plt.gca().tick_params(labelsize=15)
    plt.legend()
    # plt.axis([1e4, 5e7, -150, -104])
    plt.show()
    #
    # plt.figure()
