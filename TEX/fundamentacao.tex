...
A expressão no tempo do IRR de polo simples é mostrada na equação \ref{eq:irr_simple_pole}, o que resulta na função de transferência no domínio Z conforme a equação \ref{eq:irr_tranfer_function}.
...
\subsection{Conversão de domínio S para Z}

O operador $Z$ e definido como $Z^{j\theta}$, sendo $\theta = \omega t_0$, $\omega = 2 \pi f$ é a frequência angular e $t_0$ o período de amostragem, no caso $t_0 = 1/f_R$. Assim:

\[Z^{j\theta} = Z^{j\omega /f_R}\]
Para valores onde $\omega << f_R$ pode ser simplificado como:

\[Z^{j\omega /f_R} = cos(\omega /f_R) + jsen(\omega /f_R) = \]
Pela propriedade que $sen(x)$, para x muito pequeno é o próprio valor de X. Assim:

\[Z^{j\theta} = 1 + j\theta = 1 + \frac{j\omega}{f_R} \]
\[s = f_R(z - 1)\]


%\subsection{ADPLL type II}
...
No ADPLL type II assumindo que $K_{DCO}$ está corretamente estimado obtém-se a função de transferência de loop aberto como:
\[H_{ol}(s) = \big( \alpha + \frac{\rho f_R}{s} \big) \frac{f_R}{s}  \]
\[H_{ol}(s) = \frac{\rho f_R^2}{s} \cdot \frac{1 + s/(\rho f_R / \alpha)}{s}\]

Dois polos na origem $\omega_p1 = \omega_p2 = 0$, e um zero complexo em $\omega_z = j(\rho f_R / \alpha) $

Closed-loop 

\[H_{cl}(s) = N\frac{ ( \alpha + \rho f_R/s)(f_R/s)}{1 + (\alpha + \rho f_R/s) (f_R/s)}\]
...
\[H_{cl}(z) = N \frac{ \alpha (z - 1) + \rho}{ (z - 1)^2 + \alpha^2 (z - 1) + \rho}\]


%\subsection{Filtro IRR}


\[y[k] = (1 - \lambda)y[k - 1] + \lambda x[k]   \]

\textbf{Domínio Z}
\[H_{iir}(z) = \frac{ \lambda z}{ z - ( 1 - \lambda}\]
...
\[H_{ol}(s) = \big( \alpha + \frac{\rho f_R}{s} \big) \cdot \frac{f_R}{s} \frac{ 1 + s/f_R}{ 1 + s/\lambda f_R} \]

\[H_{ol}(s) = \frac{\rho f_R^2}{s} \cdot \frac{1 + s/(\rho f_R / \alpha)}{s} \cdot \frac{f_R}{s} \frac{ 1 + s/f_R}{ 1 + s/\lambda f_R}\]

Dois polos na origem $\omega_p1 = \omega_p2 = 0$, um polo em $\omega_p3 = j\lambda f_R$ e dois zeros em $\omega_z 1 = j(\rho f_R / \alpha)$ e $\omega_z 2 = jf_R$.

Com mais de um estágio e valores de $\lambda$ diferentes em cada:
